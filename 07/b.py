...
HIGH_CARDS_DICT = {"T": 10, "J": 1, "Q": 11, "K": 12, "A": 13}
...
def find_type(hand):
    ...
    for card, count in hand_dict.items():
        hand_tab.append([card, count])

    hand_tab.sort(key=lambda x: CARDS_DICT[x[0]], reverse=True)
    hand_tab.sort(key=lambda x: x[1], reverse=True)

    for i, (card, count) in enumerate(hand_tab):
        if card == "J" and i != 0:
            hand_tab[0][1] += count
            hand_tab.remove([card, count])
        elif card == "J" and i == 0 and len(hand_tab) > 1:
            hand_tab[1][1] += count
            hand_tab.remove([card, count])
